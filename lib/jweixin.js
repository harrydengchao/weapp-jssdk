...
export default class Jwexin {
  constructor(config = {}) {
    ...
    this.userConfig = { debug, jsApiList } // 用户配置
    ...
    return new Promise((resolve, reject) => {
      axios
        .get(this.signatureServer, {
          params: { url: signatureUrl || window.location.href.split('#')[0] }
        })
        .then((res) => {
          if (String(res.status) !== '200' || !res.data || String(res.data.code) !== '0000') {
